...
let gender; // в этой переменной будет хранится выбраный пол
...
form.onsubmit = function(e) {
    ...
    gender = document.querySelector("[data-gender]").checked; // проверяем, выбран ли женский пол: да или нет

    liverdWeeks = calcWeeks(birthday); // вычисляем количество прожитых недель
    calcRetiredAge(gender); // вычисляем возраст пенсии
    colorizeDots(livedWeeks); // закрашиваем точки-недели в таблице
    showingMiddleAge(gender); // меняем информацию о среднем возрасте слева от таблицы с точками
  };
